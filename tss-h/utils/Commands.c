...
Kyber_Decrypt_In_Unmarshal(Kyber_Decrypt_In *target, BYTE **buffer, uint32_t *size, TPM_HANDLE handles[])
{
    ...
    if (rc == TPM_RC_SUCCESS) {
        rc = TSS_TPM2B_KYBER_ENCRYPT_Unmarshalu(&target->message, buffer, size);
        if (rc != TPM_RC_SUCCESS) {
            rc += RC_Kyber_Decrypt_message;
        }
    }
...
Decapsulate_In_Unmarshal(Decapsulate_In *target, BYTE **buffer, uint32_t *size, TPM_HANDLE handles[])
{
  ...
  if (rc == TPM_RC_SUCCESS) {
    rc = TSS_TPM2B_DEC_CIPHER_TEXT_Unmarshalu(&target->cipher_text, buffer, size);
    if (rc != TPM_RC_SUCCESS) {
      rc += RC_Decapsulate_cipher_text;
    }
  }
  return rc;
}
