 ...
 * The anonymous schemes are ECDAA. ECDAA can be used to do things
 * like U-Prove. */
BOOL
	  ...
	  case TPM_ALG_DILITHIUM:
        switch(publicArea->parameters.dilithiumDetail.mode) {
			case TPM_DILITHIUM_MODE_1:						//mldsa44
			    consistent = publicArea->unique.dilithium.t.size == 0;
			    break;
			...
			case TPM_DILITHIUM_MODE_4:						//p256
			    consistent = publicArea->unique.dilithium.t.size == 33;
			    break;
			case TPM_DILITHIUM_MODE_5:						//ed2559
			    consistent = publicArea->unique.dilithium.t.size == 32;
			    break;
			case TPM_DILITHIUM_MODE_6:						//p384
			    consistent = publicArea->unique.dilithium.t.size == 49;
			    break;
			case TPM_DILITHIUM_MODE_7:						//p521
			    consistent = publicArea->unique.dilithium.t.size == 67;
			    break;
			case TPM_DILITHIUM_MODE_8:						//ed448
			    consistent = publicArea->unique.dilithium.t.size == 57;
			    break;
            default:
                consistent = FALSE;
                break;
			  ...
			  )
{
    ...
    switch(publicArea->type)
	{
	  ...
	  case TPM_ALG_DILITHIUM:
        switch(publicArea->parameters.dilithiumDetail.mode) {
			case TPM_DILITHIUM_MODE_1:						//mldsa44
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 0;
			    break;
			case TPM_DILITHIUM_MODE_2:						//mldsa65
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 0;
			    break;
			...
			case TPM_DILITHIUM_MODE_4:						//mldsa44 + p256
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 32;
			    break;
			case TPM_DILITHIUM_MODE_5:						//mldsa44 + ed2559
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 32;
			    break;
			case TPM_DILITHIUM_MODE_6:						//mldsa65 + p384
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 48;
			    break;
			case TPM_DILITHIUM_MODE_7:						//mldsa87 + p521
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 66;
			    break;
			case TPM_DILITHIUM_MODE_8:						//mldsa87 + ed448
			    consistent = sensitiveArea->sensitive.dilithium.t.size == 57;
			    break;
            default:
                consistent = FALSE;
                break;
	  ...
	  case TPM_ALG_DILITHIUM:
          switch(publicArea->parameters.dilithiumDetail.mode) {
			case TPM_DILITHIUM_MODE_1:
			  if(publicArea->unique.dilithium.t.size != 0)	//mldsa44
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_2:
			  if(publicArea->unique.dilithium.t.size != 0)	//mldsa65
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_3:
			  if(publicArea->unique.dilithium.t.size != 0)	//mldsa87
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_4:
			  if(publicArea->unique.dilithium.t.size != 33)	//mldsa44 + p256
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_5:
			  if(publicArea->unique.dilithium.t.size != 32)	//mldsa44 + ed2559
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_6:
			  if(publicArea->unique.dilithium.t.size != 49)	//mldsa65 + p384
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_7:
			  if(publicArea->unique.dilithium.t.size != 67)	//mldsa87 + p521
			      return TPM_RC_KEY + blamePublic;
			  break;
			case TPM_DILITHIUM_MODE_8:
			  if(publicArea->unique.dilithium.t.size != 57)	//mldsa87 + ed448
			      return TPM_RC_KEY + blamePublic;
			  break;
            default:
                return TPM_RCS_VALUE + blamePublic;
          ...
          if (sensitive != NULL) {
            switch(publicArea->parameters.dilithiumDetail.mode) {
				case TPM_DILITHIUM_MODE_1:
				    if (sensitive->sensitive.dilithium.t.size != 0)	//mldsa44
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_2:
				    if (sensitive->sensitive.dilithium.t.size != 0)	//mldsa65
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_3:
				    if (sensitive->sensitive.dilithium.t.size != 0)	//mldsa87
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_4:
				    if (sensitive->sensitive.dilithium.t.size == 32)	//p256
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_5:
				    if (sensitive->sensitive.dilithium.t.size != 32)	//ed2559
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_6:
				    if (sensitive->sensitive.dilithium.t.size != 48)	//p384
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_7:
				    if (sensitive->sensitive.dilithium.t.size != 66)	//p521
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
				case TPM_DILITHIUM_MODE_8:
				    if (sensitive->sensitive.dilithium.t.size == 57)	//ed448
				        return TPM_RCS_SIZE + blameSensitive;
				    break;
                default:
                    return TPM_RCS_VALUE + blamePublic;
