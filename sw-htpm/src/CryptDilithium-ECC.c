...
CryptDilithiumSign(
    ...
    size_t ecc_sk_size = (size_t)(key->sensitive.sensitive.dilithium.t.size);
...
)
{
    ...
    size_t ecc_pk_size = (size_t)(key->publicArea.unique.dilithium.t.size);
    unsigned char* ecc_pk = (unsigned char*)malloc(ecc_pk_size * sizeof(unsigned char));
    size_t ecc_sig_len = (size_t)(sig->signature.dilithium.sig.t.size);
