 ...
 **************************************************/
void shake128_fel_absorb(shake128ctx *state, const uint8_t *input, size_t inlen) {
...
/*************************************************
 * Name:        shake128_fel_squeezeblocks
 *
 ...
 **************************************************/
void shake128_fel_squeezeblocks(uint8_t *output, size_t nblocks, shake128ctx *state) {
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
void shake256_fel_absorb(shake256ctx *state, const uint8_t *input, size_t inlen) {
    state->ctx = malloc(PQC_SHAKECTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
/*************************************************
 * Name:        shake256_fel_squeezeblocks
 *
 ...
 **************************************************/
void shake256_fel_squeezeblocks(uint8_t *output, size_t nblocks, shake256ctx *state) {
...
void shake128(uint8_t *output, size_t outlen,
    ...
    shake128_fel_absorb(&s, input, inlen);
    shake128_fel_squeezeblocks(output, nblocks, &s);

    output += nblocks * SHAKE128_RATE;
    outlen -= nblocks * SHAKE128_RATE;

    if (outlen) {
        shake128_fel_squeezeblocks(t, 1, &s);
...
void shake256(uint8_t *output, size_t outlen,
    ...
    shake256_fel_absorb(&s, input, inlen);
    shake256_fel_squeezeblocks(output, nblocks, &s);

    output += nblocks * SHAKE256_RATE;
    outlen -= nblocks * SHAKE256_RATE;

    if (outlen) {
        shake256_fel_squeezeblocks(t, 1, &s);
        for (size_t i = 0; i < outlen; ++i) {
            output[i] = t[i];
        }
    }
